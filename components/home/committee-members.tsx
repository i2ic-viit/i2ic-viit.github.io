...
//             asChild
//           >
//             <Link href="/committee"><ArrowUpRight size={16} /> View All Members </Link>
//           </Button>
//           </div>
//       </div>
//     </section>
...
export default function CommitteeSection({ showViewAllButton = true }) {
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
  const controls = useAnimation();
  const sectionRef = useRef(null);
  ...
  return (
    <section ref={sectionRef} className="bg-gradient-to-b from-secondary to-background py-20 px-4 sm:px-6 lg:px-8" id='commitee'>
      <div className="container mx-auto">
        <motion.div
          className="text-center mb-16"
          initial={{ opacity: 0, y: -20 }}
        ...
        <motion.div
          className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 ${showViewAllButton?`items-center`:``} `}
          variants={containerVariants}
          initial="hidden"
          animate={controls}
        >
          {(!showViewAllButton ? committeeMembers : committeeMembers.slice(0, 8)).map((member, index)=> (
        ...
        {showViewAllButton && (
          <div className="text-center">
            <Button className="flex items-center justify-center space-x-2 max-w-sm mx-auto my-10" asChild>
              <Link href="/about#commitee">
                <span className='text-xl'>View All Members &#8599;</span>
              </Link>
            </Button>
          </div>
        )}  
